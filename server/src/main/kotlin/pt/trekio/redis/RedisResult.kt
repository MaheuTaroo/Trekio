...
sealed interface RedisResult {
    data class Success<T>(val value: T): RedisResult

    sealed class Failure(val error: String): RedisResult {
        class CouldNotPublish(cause: String): Failure("Could not publish to channel: $cause")
