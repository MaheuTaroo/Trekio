...
abstract class Api {
    ...
    protected fun protectedWithOAuth(handler: suspend RoutingContext.(OAuthAccessTokenResponse.OAuth2) -> Unit): ClassicControllerMethod =
        suspend protected@{
    ...
    protected fun classicProtectedWithPair(handler: suspend RoutingContext.(UserTokenPair) -> Unit): ClassicControllerMethod =
        suspend protected@{
    ...
    protected fun webSocketProtectedWithId(handler: suspend DefaultWebSocketServerSession.(ULong) -> Unit): WebSocketControllerMethod =
        suspend protected@{
            val auth = call.principal<ULong>()
            if (auth == null) {
