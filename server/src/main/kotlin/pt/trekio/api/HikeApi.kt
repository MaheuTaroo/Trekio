...
class HikeApi(
    ...
    @OptIn(ExperimentalAtomicApi::class)
    private fun addActiveWebSocket(tid: ULong, uid: ULong, sid: ULong) {
        wsLock.withLock {
     ...
     */
    private suspend fun WebSocketServerSession.cancelHike(uid: ULong, hid: ULong) {
     ...
     */
    private suspend fun WebSocketServerSession.startHikingSession(uid: ULong, tid: ULong): Pair<ULong, ULong>? {
        val firstLocation = try {
     ...
     */
    private suspend fun WebSocketServerSession.finishHike(uid: ULong, tid: ULong, hid: ULong, sid: ULong): Int {
        ...
        if (msg is RedisResult.Failure) {
            ...
            try {
                val endLocation =
                    parser.decodeFromString<HikerLocationNoticeDto>(
                        (msg as RedisResult.Success<*>).value as String
                    ).currentLocation.toGeoPoint()

                val finishRes = service.finishHike(
                    uid,
                    hid,
                    endLocation
                )
    ...
    private suspend fun WebSocketServerSession.reportLocation(uid: ULong, tid: ULong, sid: ULong, frame: Frame) {
        try {
            val msg = frame.data.decodeToString().toGeoPoint().toDto()
    ...
    @OptIn(ExperimentalAtomicApi::class)
    private suspend fun WebSocketServerSession.shouldStopSession(tid: ULong, sid: ULong): Boolean {
        val contCode = wsLock.withLock {
            when {
